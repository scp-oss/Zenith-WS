...
`sudo -E` обязателен — без `-E` sudo сбросит `CLOUDFLARE_API_TOKEN` из
окружения. `--env-file PATH` меняет, куда пишется env (по умолчанию
`/etc/z2r_autobench/tgrelay.env`); `--skip-redirect` — не трогать
iptables (например, если `tg-transparent-relay.service` ещё не
установлен вообще).
