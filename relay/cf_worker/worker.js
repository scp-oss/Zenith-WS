...
// WhatsApp/Meta -- см. cidr/whatsapp_ipv4.txt для полного обоснования и
// живой истории. Намеренно отдельные /32, НЕ весь AS32934 (35 блоков,
// 500k+ адресов Facebook/Instagram/Messenger) -- широкие префиксы
// (157.240.0.0/17 + 31.13.64.0/18, использовались тут до 2026-09-09)
// ловили обычный трафик Instagram, живущий в тех же подсетях, и ломали
// его REDIRECT'ом на relay/этот воркер. ДЕРЖАТЬ В ТОЧНОСТИ В СИНХРОНЕ С
// ТЕМ ФАЙЛОМ -- это два независимых копии одного списка (Worker не может
// прочитать файл из репозитория напрямую), расхождение уже было живым
// багом (список тут отставал от узкого списка на сервере несколько дней).
