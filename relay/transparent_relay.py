...
#
# Живой случай 2026-09-09 (Server A): сообщения WhatsApp по-прежнему не
# отправлялись даже после фикса выше -- изолированный tcpdump (все прочие
# приложения закрыты, только новые исходящие SYN) во время реального
# зависшего сообщения показал ДВА новых адреса, не покрытых ни этим
# множеством, ни cidr/whatsapp_ipv4.txt: 31.13.72.48 (56 попыток за ~90с,
# порты 443 И 5222 -- собственное постоянное соединение WhatsApp) и
# 157.240.200.61:443. Оба подтверждены тем же SYN-блэкхолом (`sudo -u
# wsrelay`, сырой /dev/tcp, в обход REDIRECT). См. cidr/whatsapp_ipv4.txt
# за полным разбором и `setup_redirect.sh --dports 443,5222` -- 5222 не
# ловится обычным REDIRECT-правилом по умолчанию, это отдельный фикс.
_KNOWN_BLOCKED_IPS = frozenset({
    # 31.13.72.5 (edge-mqtt.facebook.com) НАМЕРЕННО не в этом множестве --
    # подтверждено, что прямое TCP до него НЕ блокируется (см.
    # cidr/whatsapp_ipv4.txt), просто числится в REDIRECT-списке на
    # случай, если это когда-то изменится. Добавление сюда IP, который
    # прямую попытку проходит нормально, только зря гонит рабочий трафик
    # через Worker fallback без нужды.
    '31.13.72.48',    # WhatsApp собственное постоянное соединение (443 и 5222)
    '31.13.72.52',    # static.whatsapp.net / graph.whatsapp.com / api.whatsapp.net
    '157.240.0.60',   # web.whatsapp.com
    '157.240.200.61', # ещё один Meta/WhatsApp API edge
})
